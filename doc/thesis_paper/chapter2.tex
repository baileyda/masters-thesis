...
\centering
	\subfigure[]{
		\includegraphics[scale=0.25]{images/network_vis/rad_pat_certain_small_scale.eps}
		\label{fig:rad_pat_certain_zoom}
	}
	\subfigure[]{
		\includegraphics[scale=0.25]{images/network_vis/rad_pat_uncertain_small_scale.eps}
		\label{fig:rad_pat_uncertain_zoom}
	}
%\end{center}
\caption{Simplified versions of the signal radiation pattern visualizations for the two channel state models, \subref{fig:rad_pat_certain_zoom} fully known channel state, \subref{fig:rad_pat_uncertain_zoom} highly unstable channel state.}
\label{fig:rad_pat_zoom}
\end{figure}
...
\begin{center}
	\subfigure[]{
		\includegraphics[scale=0.25]{images/network_vis/rad_pat_certain_small_scale_sinr.eps}
		\label{fig:rad_pat_sinr_certain_zoom}
	}
	\subfigure[]{
		\includegraphics[scale=0.25]{images/network_vis/rad_pat_uncertain_small_scale_sinr.eps}
		\label{fig:rad_pat_sinr_uncertain_zoom}
}
\end{center}
\caption{Acceptable SINR regions with signal radiation patterns for the two channel state models, \subref{fig:rad_pat_sinr_certain_zoom} fully known channel state, \subref{fig:rad_pat_sinr_uncertain_zoom} highly unstable channel state.}
\label{fig:rad_pat_sinr_zoom}
\end{figure}

In visualizing these regions of acceptable SINR we can also see that the presence of uncertainty makes it difficult to design node movement and scheduling such that the receiver nodes can remain in the feasible regions of the transmitter with whom they seek to communicate.
